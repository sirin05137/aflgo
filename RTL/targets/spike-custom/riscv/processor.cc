...
void processor_t::reset()
{
  ...
  memset(&pcode, 0, sizeof(struct pcode_csr));
  if (ext)
...
void processor_t::set_csr(int which, reg_t val)
{
  ...
  switch (which)
  {
    ...
    case CSR_DSCRATCH:
      state.dscratch = val;
      break;
	case CSR_ISOLATIONSW:
		pcode.isolationSW = val;
		fprintf(stdout, "[DEBUG] isolation sw on! %x\n", pcode.isolationSW);
		break;
	case CSR_STOREMONITOR:
		mem_list[m_index] = (unsigned long)val;	
		fprintf(stdout,"[mem check] mem_list[%x] = %lx\n", (unsigned long)m_index, (unsigned long)mem_list[m_index]);
		m_index += 1;
		break;
	case CSR_VPOFFSET:
		pcode.vpoffset = val;
		fprintf(stdout, "csr_vpoffset: %lx\n", pcode.vpoffset);
		break;
    /*
	case CSR_SPCODELOCK:
      //fprintf(stdout,"attempting to update pcodelock from %x to %lx\n",pcode.pcodelock, val);
      if (pcode.pcodelock & 0x1) break;
      pcode.pcodelock = val;
      //fprintf(stdout,"pcodelock updated to: %x\n",pcode.pcodelock);
      break;
	*/
    case CSR_SPCODEBASE0:
          pcode.pcode_region[0].mask = (val & 0xffc) >> 2;
          fprintf(stdout,"pcode_region[0].mask updated to: %x\n",pcode.pcode_region[0].mask);
          pcode.pcode_region[0].base = (val >> 12);
          fprintf(stdout,"pcode_region[0].base updated to: %x\n",pcode.pcode_region[0].base);
		  pcode.pcode_region[0].valid = (val & 0x2) >> 1;
	      fprintf(stdout,"pcode_region[0].valid updated to: %x\n",pcode.pcode_region[0].valid);
		  pcode.pcode_region[0].lock = val & 0x1;
         fprintf(stdout,"pcode_region[0].rock updated to: %x\n",pcode.pcode_region[0].lock);
		break;

	case CSR_SPCODEBASE1:
          pcode.pcode_region[1].mask = (val & 0xffc) >> 2;
          fprintf(stdout,"pcode_region[1].mask updated to: %x\n",pcode.pcode_region[1].mask);
          pcode.pcode_region[1].base = (val >> 12);
          fprintf(stdout,"pcode_region[1].base updated to: %x\n",pcode.pcode_region[1].base);
		      pcode.pcode_region[1].valid = (val & 0x2) >> 1;
	        fprintf(stdout,"pcode_region[1].valid updated to: %x\n",pcode.pcode_region[1].valid);
		      pcode.pcode_region[1].lock = val & 0x1;
          fprintf(stdout,"pcode_region[1].rock updated to: %x\n",pcode.pcode_region[1].lock);
	
      break;

 case CSR_SPCODEBASE2:
          pcode.pcode_region[2].mask = (val & 0xffc) >> 2;
          fprintf(stdout,"pcode_region[2].mask updated to: %x\n",pcode.pcode_region[2].mask);
          pcode.pcode_region[2].base = (val >> 12);
          fprintf(stdout,"pcode_region[2].base updated to: %x\n",pcode.pcode_region[2].base);
		  pcode.pcode_region[2].valid = (val & 0x2) >> 1;
	      fprintf(stdout,"pcode_region[2].valid updated to: %x\n",pcode.pcode_region[2].valid);
		  pcode.pcode_region[2].lock = val & 0x1;
         fprintf(stdout,"pcode_region[2].rock updated to: %x\n",pcode.pcode_region[2].lock);
	
      break;


    case CSR_SPCODEBASE3:
          pcode.pcode_region[3].mask = (val & 0xffc) >> 2;
          fprintf(stdout,"pcode_region[3].mask updated to: %x\n",pcode.pcode_region[3].mask);
          pcode.pcode_region[3].base = (val >> 12);
          fprintf(stdout,"pcode_region[3].base updated to: %x\n",pcode.pcode_region[3].base);
		  pcode.pcode_region[3].valid = (val & 0x2) >> 1;
	      fprintf(stdout,"pcode_region[3].valid updated to: %x\n",pcode.pcode_region[3].valid);
		  pcode.pcode_region[3].lock = val & 0x1;
         fprintf(stdout,"pcode_region[3].rock updated to: %x\n",pcode.pcode_region[3].lock);
	      break;
	 }
}
...
reg_t processor_t::get_csr(int which)
{
  ...
  switch (which)
  {
    ...
    case CSR_ISOLATIONSW:
	  return pcode.isolationSW;
	case CSR_STOREMONITOR:
	case CSR_VPOFFSET:
	  return pcode.vpoffset;
    case CSR_SPCODEBASE0:
    fprintf(stderr, "[spike] ret: %x\n", ((pcode.pcode_region[0].base << 12) | (pcode.pcode_region[0].mask << 2)| (pcode.pcode_region[0].valid << 1 ) | (pcode.pcode_region[0].lock)));
    fprintf(stderr, "[spike] base[0]: %x\n", pcode.pcode_region[0].base);
    fprintf(stderr, "[spike] mask[0]: %x\n", pcode.pcode_region[0].mask);
    fprintf(stderr, "[spike] valid[0]: %x\n", pcode.pcode_region[0].valid);
    fprintf(stderr, "[spike] lock[0]: %x\n", pcode.pcode_region[0].lock);
		return ((pcode.pcode_region[0].base << 12) | (pcode.pcode_region[0].mask << 2) | (pcode.pcode_region[0].valid << 1 ) | (pcode.pcode_region[0].lock));
    case CSR_SPCODEBASE1:
    fprintf(stderr, "[spike] ret: %x\n", ((pcode.pcode_region[1].base << 12) | (pcode.pcode_region[1].mask << 2)| (pcode.pcode_region[1].valid << 1 ) | (pcode.pcode_region[1].lock)));
    fprintf(stderr, "[spike] base[1]: %x\n", pcode.pcode_region[1].base);
    fprintf(stderr, "[spike] mask[1]: %x\n", pcode.pcode_region[1].mask);
    fprintf(stderr, "[spike] valid[1]: %x\n", pcode.pcode_region[1].valid);
    fprintf(stderr, "[spike] lock[1]: %x\n", pcode.pcode_region[1].lock);
		return ((pcode.pcode_region[1].base << 12) | (pcode.pcode_region[1].mask << 2) | (pcode.pcode_region[1].valid << 1 ) | (pcode.pcode_region[1].lock));
    case CSR_SPCODEBASE2:
    fprintf(stderr, "[spike] ret: %x\n", ((pcode.pcode_region[2].base << 12) | (pcode.pcode_region[2].mask << 2)| (pcode.pcode_region[2].valid << 1 ) | (pcode.pcode_region[2].lock)));
		return ((pcode.pcode_region[2].base << 12) | (pcode.pcode_region[2].mask << 2) | (pcode.pcode_region[2].valid << 1 ) | (pcode.pcode_region[2].lock));
    case CSR_SPCODEBASE3:
    fprintf(stderr, "[spike] ret: %x\n", ((pcode.pcode_region[3].base << 12) | (pcode.pcode_region[3].mask << 2)| (pcode.pcode_region[3].valid << 1 ) | (pcode.pcode_region[3].lock)));
		return ((pcode.pcode_region[3].base << 12) | (pcode.pcode_region[3].mask << 2) | (pcode.pcode_region[3].valid << 1 ) | (pcode.pcode_region[3].lock));
    
  }
  //fprintf(stdout, "illegal insn on csr access: %x\n", which); 
  throw trap_illegal_instruction(0);
}

reg_t illegal_instruction(processor_t* p, insn_t insn, reg_t pc)
{
    fprintf(stdout,"illegal instruction: %lx, %lx\n",insn.bits(), pc); 
  throw trap_illegal_instruction(0);
}
