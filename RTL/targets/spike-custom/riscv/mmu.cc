...
tlb_entry_t mmu_t::fetch_slow_path(reg_t vaddr)
{
  ...
  if (pcode_enabled() && !within_pcode(paddr) && proc->get_state()->prv ==PRV_S && proc->get_state()->prv != PRV_M) {
      fprintf(stdout,"[spike-pcl] detected pcode violation on fetch @ %lx -> %lx, prv: %x (PRV_S: %x)\n", vaddr, paddr, proc->get_state()->prv, PRV_S);
      throw trap_instruction_page_fault(vaddr);

    if(!((vaddr - paddr) == pcode->vpoffset || (paddr - vaddr) == pcode->vpoffset)) {
      		fprintf(stdout, "[OFFSET TEST] kernel offset and real offset not matching: %lx, %lx, %lx, %lx, %lx\n", vaddr, paddr, pcode->vpoffset, vaddr-paddr, paddr-vaddr);
          throw trap_instruction_page_fault(vaddr);
    }
...
/*
  if(!isw_disabled()) {
	  for (int i=0; i<m_index; i = i+2) {
		  if(mem_list[i] < paddr && (mem_list[i] + mem_list[i+1]) > paddr) {
			  fprintf(stdout, "[mem check]: FETCH! There is fetch instruction in protection mem @ %lx\n", paddr); 
			   f_count += 1;
			   fprintf(stdout, "fetch count : %d\n", f_count);
			  break;
...
*//*
  reg_t real_offset;
  if(!isw_disabled() && proc->get_state()->prv == PRV_S) {
	real_offset = vaddr - paddr;
	if( real_offset == pcode->vpoffset) 
		fprintf(stdout, "[OFFSET TESET ] kernel offset and real offset was matched !\n");
	else
		fprintf(stdout, "[OFFSET TEST X] %lx\n", real_offset); 	
	}*/
...
void mmu_t::load_slow_path(reg_t addr, reg_t len, uint8_t* bytes)
{
  ...
  if (pcode_enabled() && within_pcode(paddr) && proc->get_state()->prv != PRV_M) {
      //if (proc->get_state()->prv != PRV_S) {
//        fprintf(stdout,"[spike-pcl] detected pcode violation on load due to user read@ %lx -> %lx\n", addr, paddr);
 //         throw trap_load_page_fault(addr);
  //    }
    //fprintf(stdout,"[spike-pcl] detected pcode violation on load @ %lx -> %lx\n", addr, paddr);
    //pcode->set_load_fault(); 
...
if(!isw_disabled()) {
	  for (int i=0; i<m_index; i = i+2) {
		  if(mem_list[i] < paddr && (mem_list[i] + mem_list[i+1]) > paddr) {
			  fprintf(stdout, "[mem check]: LOAD! There is store instruction in protection mem @ %lx\n", paddr); 
			  l_count += 1;
...
void mmu_t::store_slow_path(reg_t addr, reg_t len, const uint8_t* bytes)
{
  ...
  //if (pcode_enabled() && (paddr >= 0x81000000) && (paddr < 0x80200000)) {
//    fprintf(stdout,"[spike-pcl] enabled: %x, paddr: %x, within_pcode: %x\n", pcode_enabled(), paddr, within_pcode(paddr));
//  }


  if (pcode_enabled() && within_pcode(paddr) && proc->get_state()->prv != PRV_M) {
      fprintf(stdout,"[spike-pcl] detected pcode violation on store @ %lx -> %lx\n", addr, paddr);
      //pcode->set_store_fault(); 
...
/*
  if(!isw_disabled()) { 
	  for (int i=0; i<m_index; i = i+2) {
		  ...
		  if(mem_list[i] < paddr && (mem_list[i] + mem_list[i+1]) > paddr) {
			  fprintf(stdout, "[mem check]: STORE! There is store instruction in protection mem @ %lx\n", paddr); 
			   s_count += 1;
